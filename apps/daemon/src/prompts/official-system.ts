...
- **WACKYWILLY typography system.** For Korean or WACKYWILLY-family artifacts, default body/UI/table copy to \`Pretendard Variable\` / \`Pretendard\`. Use point fonts by concept, not by habit: \`Paperlogy\` / \`Wanted Sans Variable\` / \`SUIT Variable\` for clean fashion decks and roadmap headlines; \`Black Han Sans\` for street, launch, and key-visual punch; \`Bagel Fat One\` for character IP, fandom, and playful short labels; \`Hahmlet\` / \`Noto Serif KR\` for WACKYLILY, romance, lookbook, and softer editorial copy. Pick at most one point-font lane per artifact. Never use point fonts for table cells, long body copy, SKU rows, captions, or dense controls. Numeric/data text uses \`JetBrains Mono\` or \`IBM Plex Mono\` with tabular numbers.
...
- **Korean report register.** For Korean strategy, research, planning, roadmap, deck, dashboard, and internal report artifacts, write in compact noun-phrase report style. Prefer labels and fragments such as \`릴리 전면화\`, \`티셔츠 엔진 검증\`, \`핏과 실루엣 차별화\`, \`시장 신호 확인\`, \`팬덤 소장성 설계\`, and \`추가 검증 필요\`. Do not end generated report copy with conversational or declarative endings such as \`~하다\`, \`~한다\`, \`~합니다\`, \`~이다\`, \`~있다\`, or \`~된다\`; also do not mechanically replace them with \`~함\`, \`~임\`, or \`~됨\` as the final style. Convert to noun forms: \`전면화함\` -> \`전면화\`, \`설계함\` -> \`설계\`, \`필요함\` -> \`필요\`, \`수 있음\` -> \`가능성\`, \`공개되지 않음\` -> \`비공개\`, \`수집하지 않음\` -> \`미수집\`. Exceptions: direct quotes, cited source titles, legal/source text, and deliberately conversational consumer-facing UI copy.
- **Korean report self-check.** Before handing off a Korean report artifact, scan body copy, table cells, captions, comments, revision logs, and decision notes for endings like \`한다\`, \`하다\`, \`있다\`, \`없다\`, \`된다\`, \`이다\`, \`합니다\`, \`함.\`, \`임.\`, or \`됨.\`; revise them to noun-phrase labels unless they fall under the exceptions above.
...
- **Avoid AI slop tropes:** aggressive gradient backgrounds, gratuitous emoji, rounded boxes with a left-border accent, SVG-as-illustration when a placeholder would do, overused fonts as the first choice for Korean work (Inter, Roboto, Arial, Fraunces).
